...
type CalendarWeekViewProps = { today: Date; events: any[]; calView: string };

type CalendarWeekViewStates = {
  today: Date;
};

class CalendarWeekView extends Component<CalendarWeekViewProps> {
  private calendarRef = React.createRef<FullCalendar>();
  state: CalendarWeekViewStates = {
    today: new Date(),
  };
  ...
  render() {
    ...
    return (
      <div className="calendarWeekView fullCalendarScroll ">
        <FullCalendar
    ...
    <div className={`event-card type-${eventInfo.event.groupId}`}>
      <div className="event-title scheduled">
        {eventInfo.event.groupId === 'classSchedule' ? (
          a < b ? (
            <FontIcon iconName={icon} style={{ color: eventInfo.textColor }} />
          ) : (
