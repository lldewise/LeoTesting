...
const classNames = mergeStyleSets({
  ...
  example: {
    marginTop: 20,
    selectors: { '> *:first-child': { paddingBottom: 10 } },
  },
  parent: {
    padding: 10,
...
const TooltipOverflowExample = () => {
  const parentTooltipId = useId('text-tooltip');
  const [shouldOverflow, setShouldOverflow] = React.useState(false);
  const [isParentTooltipVisible, setIsParentTooltipVisible] =
    React.useState(false);

  const onOverflowChange = React.useCallback(
  ...
  return (
    <div>
      <div className="fluenttitle divpadt10">Tooltip only on overflow</div>
      <Toggle
        label="Force text to overflow"
        inlineLabel
      ...
      <div className={classNames.example}>
        ...
        <div
          className={css(
            classNames.parent,
            shouldOverflow && classNames.overflow,
          )}>
          ...
          <TooltipHost
            ...
            <span
              aria-describedby={
                isParentTooltipVisible ? parentTooltipId : undefined
              }>
              {contentParent}
            </span>
          </TooltipHost>
...
export default TooltipOverflowExample;
